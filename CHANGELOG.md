...
- **sdk**: warn for reporting score when not using Iftracer (#829)
...
- **iftracer-sdk**: default value for metrics endpoint (#711)
...
- **iftracer-sdk**: custom span processor's on_start is honored (#695)
...
- **iftracer-sdk**: Log error message when providing wrong API key. (#638)
...
- **iftracer-sdk**: support explicit prompt versioning in prompt management (#221)
...
- **iftracer-sdk**: logging of service name in traces (#99)
...
- **iftracer-sdk**: missing lockfile (#72)
- **iftracer-sdk**: flushing in notebooks (#66)
- py security issue
- **docs**: update exporting.mdx to include nr instrumentation (#12)
- **sdk**: async decorators not awaited
- **sdk**: missing dependency
- warn if Iftracer wasn't initialized properly (#11)
- match new dashboard API
- **iftracer-sdk**: duplicate spans reporting (#10)
- moved api key to /tmp
- /v1/traces is always appended to endpoint
- parse headers correctly
- **iftracer-sdk**:  replace context variables with otel context + refactor (#8)
